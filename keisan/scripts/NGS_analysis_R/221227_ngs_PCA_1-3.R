...
# datに発現量のmatrixを代入 
sample <- colnames(tmm)[grep(".*[1-3]$", colnames(tmm))]
setwd("PCA1_3_log2")

dat <- tmm # tmm使うとき
dat <- tpm[rownames(tmm), sample] # tpm使うとき
dat <- dat[apply(dat, 1, min)>0, ]
dat <- log2(dat)
...
# datに発現量のmatrixを代入 
sample <- colnames(tmm)[grep(".*[7-9]$", colnames(tmm))]
setwd("PCA7_9")
